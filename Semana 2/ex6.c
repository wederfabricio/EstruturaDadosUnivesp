//Arquivo obtido pela UNIVESP, adaptado para o exercício
/******************************************************************************
...
int maiorChave(LISTA l) {
    if(l.cabeca->prox == l.cabeca) return -1;
    int maiorChave;
    PONT prox = l.cabeca->prox;
    while(prox != l.cabeca) {
        if(maiorChave < prox->reg.chave) {
            maiorChave = prox->reg.chave;
        }
        prox = prox->prox;
    }
    return maiorChave;
